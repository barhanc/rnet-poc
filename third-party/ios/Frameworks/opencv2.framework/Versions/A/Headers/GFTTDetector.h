...
//
// static Ptr_GFTTDetector cv::GFTTDetector::create(int maxCorners, double qualityLevel, double minDistance, int blockSize, int gradientSize, bool useHarrisDetector = false, double k = 0.04)
//
+ (GFTTDetector*)create:(int)maxCorners qualityLevel:(double)qualityLevel minDistance:(double)minDistance blockSize:(int)blockSize gradientSize:(int)gradientSize useHarrisDetector:(BOOL)useHarrisDetector k:(double)k NS_SWIFT_NAME(create(maxCorners:qualityLevel:minDistance:blockSize:gradientSize:useHarrisDetector:k:));

+ (GFTTDetector*)create:(int)maxCorners qualityLevel:(double)qualityLevel minDistance:(double)minDistance blockSize:(int)blockSize gradientSize:(int)gradientSize useHarrisDetector:(BOOL)useHarrisDetector NS_SWIFT_NAME(create(maxCorners:qualityLevel:minDistance:blockSize:gradientSize:useHarrisDetector:));

+ (GFTTDetector*)create:(int)maxCorners qualityLevel:(double)qualityLevel minDistance:(double)minDistance blockSize:(int)blockSize gradientSize:(int)gradientSize NS_SWIFT_NAME(create(maxCorners:qualityLevel:minDistance:blockSize:gradientSize:));
